...
Experiment advisor is the next-step planner. It reads the current dataset, triage report, validation metrics, trial history, and any diagnostics you have run, including data-acquisition, leakage-sentinel, learning-curve, calibration-slice, and external-holdout evidence, then ranks concrete follow-up experiments: clean data first, collect needed labels, quarantine possible leakage, collect labels where the curve is still rising, train a baseline, expand the search, tune thresholds, repair calibration, test robustness, or validate against drift. Fresh data-acquisition, leakage-sentinel, learning-curve, calibration-slice, or external-holdout results invalidate older advisor and promotion-gate summaries so exports do not mix new evidence with stale recommendations. The `Experiment advisor lab` preset has nonlinear weak single-feature structure, imbalance, boundary flips, and outliers so the advisor naturally recommends triage plus RFF/SMOTE/CV-style runs.
