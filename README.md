...
- Post-hoc label sensitivity diagnostics that flip one loaded label at a time against fixed active-model probabilities to rank metric-sensitive labels, suspect flips, and trusted anchors.
- Label-noise stress diagnostics that retrain temporary fallback models after controlled training-label flips, then measure clean-validation F1, accuracy, Brier, log-loss, and calibration degradation.
...
- Subgroup disparity diagnostics for proxy numeric slices, complement comparisons, and error-rate gaps.
- Threshold tradeoff sweeps for operating-point, precision/recall, and cost-sensitive decisions.
- Threshold stability bootstraps best-F1 operating thresholds to show whether the active cutoff is robust.
- Capacity planner ranks loaded rows by active-model probability and simulates finite review/action budgets with precision@k, recall capture, lift, and simple net utility.
- Rank lift diagnostics build cumulative gains, lift, decile response, score-concentration, and top-row tables for review queues where ranking quality matters more than one threshold.
...
- Leakage sentinel scans loaded feature columns before training for direct label-code candidates, near-perfect single-feature separators, and low-cardinality proxy mappings.
...
- One-click dataset triage that combines audit, leakage sentinel, separability, prototype, neighborhood, and model-free OOD checks into a readiness score and action queue.
- Validation plan diagnostic that recommends collect-more-labels, stratified holdout, stratified k-fold, or chronological holdout from sample size, class counts, imbalance, and row-order drift signals.
- Validation stability audit that repeats stratified outer folds with inner calibration splits, then reports fixed- and tuned-threshold metric distributions, weak folds, and the rows in the worst validation folds.
- Data acquisition planner that turns class balance, boundary-like rows, tail coverage, and weak feature evidence into a prioritized next-label budget.
- Data value scout that ranks high-value anchors, possible harmful rows, duplicate weight, and rare valid coverage cases for row-level curation.
...
- Promotion gate that turns the current evidence bundle into a model-readiness verdict, blockers, cautions, and required release-note items.
...
- Reliability atlas for persisted calibration-bin inspection, probability communication risk, and over/underconfidence review.
- Calibration slice diagnostics for finding raw feature ranges where probabilities are locally misleading.
...
- Prior-shift prevalence simulation for PPV, NPV, alert volume, and false-positive counts under deployment base-rate changes.
- External holdout evaluation for scoring a separate labeled CSV without replacing the loaded training dataset.
...
- Shadow replay diagnostics for fixed-model row-order degradation windows and consecutive error runs.
...
- MPS site-order sweep that retrains temporary MPS models over original, reversed, correlation-driven, and seeded-random feature orders to expose site-order sensitivity.
- Dataset **audit** (imbalance, duplicates, constant/correlated features) in the workbench and reports.
- **Learning curve** diagnostics that retrain on increasing stratified fractions, evaluate one fixed holdout, and save the F1/accuracy curve with a next-step verdict.
- Trial-history export and inspection for comparing auto-experiment runs.
...
MPS order sweep is an order-sensitivity diagnostic for the `mps` backend. It retrains small temporary MPS models with a bounded set of candidate feature-site orders, reports the validation delta versus the original column order, and stores an adoption note. It does not mutate the active model and it is not feature importance. To adopt a non-original order, reorder CSV columns or manual vectors consistently and retrain/save a new MPS model. The `MPS locality lab` preset uses adjacent interacting feature pairs so this workflow is easy to test.

Validation plan is a model-free split-strategy check to run before training or after importing a new dataset. It recommends when to collect more labels, when stratified k-fold is safer than a small holdout, when a regular stratified holdout is adequate, and when ordered rows should be evaluated with chronological holdout before shuffled validation. The `Validation plan lab` preset preserves reference/current row order so this recommendation is visible.

Validation stability audit measures how much the model conclusion changes across repeated stratified outer folds. Every outer training partition gets its own inner calibration split, so preprocessing, early stopping, and threshold selection remain isolated from the untouched outer fold. It reports fixed- and tuned-threshold distributions, repeat summaries, weak folds, and worst-fold row indices. The displayed q10-q90 ranges are empirical split-sensitivity intervals, not confidence intervals. The `Split lottery lab` preset contains a rare interaction regime designed to expose conclusions that depend too strongly on a fortunate split.

Learning curve diagnostics train temporary models on increasing stratified fractions of the loaded data while scoring the same fixed holdout each time. The summary labels the curve as stable, data-limited, underfit/noisy, or needing capacity/regularization review, then stores that evidence in reports and model sidecars. The `Learning curve lab` preset adds boundary-heavy rows so the curve can show whether more reviewed labels are likely to help.

Data acquisition planner is a model-free next-label planner. It estimates whether the dataset needs broader seed rows, more minority-class examples, boundary-region labels, or feature-tail coverage before more tuning. The report stores a suggested label budget, ranked recommendations, row candidates, and feature targets. The `Data acquisition lab` preset intentionally mixes imbalance, boundary rows, and tail cases so the planner produces a concrete collection queue.

Data value scout is a model-free row-curation diagnostic. It approximates sample value from standardized nearest-neighbor support, opposing-neighbor harm, duplicate weight, and feature-tail coverage, then separates high-value anchors, review rows, redundant rows, and rare coverage rows. The `Data value scout lab` preset includes repeated anchors, exact-feature label conflicts, and rare valid tail cases so users can practice deciding what to keep, review, downweight, or turn into canaries.

Experiment reports can be exported before training when a dataset is loaded, so model-free audits are easy to archive. Reports include dataset availability, class counts when a dataset is loaded, the selected threshold, fixed-`0.5` baseline metrics, calibration diagnostics, reliability-atlas bins, calibration-slice diagnostics, post-hoc calibration repair diagnostics, post-hoc permutation-null diagnostics, post-hoc label-sensitivity diagnostics, label-noise stress diagnostics, population drift diagnostics, prior-shift prevalence simulations, external holdout evaluation, adversarial validation diagnostics, chronological holdout diagnostics, learning-curve fixed-holdout summaries, repeated validation-stability fold distributions, data-acquisition plans, data-value scout row-curation findings, shadow-replay windows, threshold-stability intervals, capacity-planner budget tables, rank-lift gains tables, schema-guard feature contracts, leakage-sentinel proxy checks, validation-plan split recommendations, canary-suite regression probes, policy-guard monotonic probes, conformal-style uncertainty diagnostics, post-hoc conformal prediction-set diagnostics, feature importance, feature ablation diagnostics, model response diagnostics, pairwise interaction diagnostics, subgroup disparity diagnostics, decision-curve utility diagnostics, selective prediction risk-coverage diagnostics, error-atlas buckets, dataset cartography, dataset triage, experiment-advisor recommendations, trial-inspector search summaries, promotion-gate verdicts, neighborhood hardness diagnostics, feature separability diagnostics, prototype audit diagnostics, OOD sentinel rows, bootstrap stability diagnostics, MPS bond sweeps, MPS site-order sweeps, and trial history for auto experiments.
...
Reliability atlas diagnostics turn the active model's probabilities on the loaded dataset into persisted calibration-bin evidence: ECE, max bin error, worst bins, overconfident and underconfident regions, sparse-bin warnings, and probability-communication recommendations. The `Reliability atlas lab` preset creates separate overconfident and underconfident bands so calibration problems are visible even when aggregate accuracy looks acceptable.

Calibration slice diagnostics scan raw feature ranges and rank slices where mean predicted probability diverges from observed positive rate. The output includes local Brier score, ECE, confidence gap, over/underconfidence direction, weighted calibration impact, and recommendations. The `Calibration slice lab` preset creates a high-score pocket with low positive prevalence so localized probability trust issues are visible even when the global model signal is useful.
...
Prior-shift diagnostics simulate deployment prevalence changes while holding the active threshold's observed sensitivity and specificity fixed. The output shows FPR/FNR, PPV, NPV, predicted-positive rate, false-discovery rate, and expected TP/FP/FN/TN per 1,000 cases across a prevalence grid. This is useful for rare-event workflows where validation prevalence may be much higher than production prevalence. The `Prior shift lab` preset includes a moderate-prevalence validation set with false-positive shoulder rows so base-rate sensitivity is visible.

External holdout evaluation scores a separate labeled CSV with the active model and active threshold without replacing the loaded training dataset. It reports F1, balanced accuracy, confusion counts, Brier/log loss/ECE, and a simple reference comparison against the loaded dataset's feature means and label prevalence. The `External holdout lab` preset creates a deployment-like shifted marker and a changed label mix so the workflow can exercise external validation before saving a model.
...
Shadow replay diagnostics keep the active model fixed and replay its probabilities across the loaded row order. The output segments rows into ordered windows, compares each window with the first window, flags F1/accuracy/Brier degradation, lists consecutive error runs, and stores an order-sensitive dataset fingerprint so stale replay evidence is not reused on shuffled rows. The `Shadow replay lab` preset keeps early, transition, and late regimes in order to exercise this fixed-model monitoring workflow.
...
Post-hoc label sensitivity keeps the active model's probabilities fixed and flips one loaded label at a time to estimate how much F1, accuracy, balanced accuracy, Brier score, log loss, and ECE would move. It ranks suspect rows where the opposite label better matches the current predictions, plus anchor rows where flipping would materially damage evidence. The `Label sensitivity lab` preset plants high-confidence label flips and trusted anchors so users can practice human label review without confusing this diagnostic for automatic relabeling.

Label-noise stress complements label sensitivity by retraining small temporary NumPy models after flipping bounded fractions of training-split labels, then scoring the original clean validation split. It answers a different question: whether small amounts of synthetic label corruption make the validation story collapse. The `Label noise stress lab` preset includes clean margins and boundary-review rows so users can see the stress curve without importing a separate dataset.
...
Model response diagnostics sweep one raw feature at a time across an observed quantile grid, hold the other row values fixed, and report mean probability curves. The output ranks features by response range, signed change, monotonicity, and nonmonotonic direction changes. The `Response curve lab` preset contains a monotonic driver, an arc-shaped nonlinear feature, and a weak-noise column so partial-dependence behavior is visible without extra dependencies.

Pairwise interaction diagnostics sweep two raw features at a time across a small observed grid and estimate how much the joint response departs from an additive combination of the two one-feature responses. The output ranks pairs by interaction strength, maximum absolute interaction, threshold crossings, and correlation warnings. The `Interaction surface lab` preset uses an `x1 * x2`-style signal so the pairwise surface should dominate weak single-feature distractors.
...
Subgroup disparity diagnostics compare each automatically derived numeric slice with its complement group. The audit reports predicted-positive-rate gaps, false-negative-rate gaps, false-positive-rate gaps, recall gaps, accuracy gaps, and compact warnings for one-class or tiny slices. The `Subgroup disparity lab` preset is built around a visible marker group with different error behavior, so the diagnostic can surface a proxy subgroup concern without claiming protected-class compliance evidence.
...
Threshold stability bootstraps the active model's probability outputs, reselects the best-F1 threshold on each resampled labeled dataset, and reports the median threshold, 5-95% interval, spread, current-threshold inclusion, and F1 gain over the current cutoff. The `Threshold stability lab` preset creates a dense boundary band so the interval can reveal whether the chosen operating point is genuinely stable or just a sample artifact.

Capacity planner treats the active model as a ranked queue. It sorts loaded rows by predicted probability, evaluates top-k budgets, reports precision@k, recall captured, lift over prevalence, false positives, threshold floor, and a simple net utility model. The `Capacity planner lab` preset adds rare positives plus false-alarm decoys so you can test whether the best model actually creates a useful action queue under finite review capacity.

Rank lift diagnostics focus on ranking evidence rather than utility costs. They sort loaded rows by active-model probability, report top-fraction gains and lift, decile response rates, normalized cumulative-gains area, score Gini, score-mass capture, and top rows. This is useful for triage, collections, outreach, moderation, or any workflow where only the top slice can be reviewed. The `Rank lift lab` preset concentrates positives near the top of a score feature while adding mid-rank decoys so gains-table behavior is visible.
...
Dataset triage is the fastest pre-training check: it runs the dataset audit, leakage sentinel, feature separability lens, prototype audit, neighborhood hardness, and model-free OOD sentinel together, then compresses the findings into a readiness score, risk level, blocker count, and prioritized next actions. Running triage also stores the component reports so a later report or model sidecar can include the detailed evidence. The `Dataset triage lab` preset intentionally includes conflicts, a constant column, redundant features, tails, and hard neighborhoods so the action queue is visible.

Schema guard is a lightweight feature-contract profiler for numeric vectors. It records per-feature ranges, quantiles, median/MAD bounds, unique counts, low-cardinality value previews, outlier counts, and contract warnings; prediction vectors are checked against the latest schema guard report so unseen low-cardinality codes or out-of-observed-range values are visible before the probability is trusted. The `Schema guard lab` preset contains wide-scale amounts, low-cardinality status codes, near-constant sensors, sparse indicators, and tail probes.

Leakage sentinel is a model-free pre-training scan for target leakage and shortcut proxies. It ranks each numeric feature by one-feature AUC, best threshold balanced accuracy, low-cardinality label mapping, pure label-coded values, and a compact risk score. High-risk findings feed Dataset triage, Experiment advisor, Promotion gate, reports, and model sidecars. The `Leakage sentinel lab` preset includes a direct label-code candidate plus a conflicted proxy bucket so users can practice quarantining suspicious columns before model selection.
...
Experiment advisor is the next-step planner. It reads the current dataset, triage report, validation metrics, trial history, and any diagnostics you have run, including data-acquisition, data-value, label-sensitivity, leakage-sentinel, learning-curve, validation-stability, calibration-slice, and external-holdout evidence, then ranks concrete follow-up experiments: clean data first, review sensitive labels, collect needed labels, curate high-impact rows, quarantine possible leakage, collect labels where the curve is still rising, stabilize split-sensitive validation, train a baseline, expand the search, tune thresholds, repair calibration, test robustness, or validate against drift. Fresh data-acquisition, data-value, label-sensitivity, leakage-sentinel, learning-curve, validation-stability, calibration-slice, or external-holdout results invalidate older advisor and promotion-gate summaries so exports do not mix new evidence with stale recommendations. The `Experiment advisor lab` preset has nonlinear weak single-feature structure, imbalance, boundary flips, and outliers so the advisor naturally recommends triage plus RFF/SMOTE/CV-style runs.
...
Promotion gate is the pre-save readiness check. It combines dataset size, class balance, validation metrics, calibration, threshold gain, dataset triage, data-acquisition gaps, data-value curation findings, label-sensitivity review findings, label-noise stress evidence, leakage-sentinel findings, learning-curve evidence, validation-stability evidence, experiment-advisor recommendations, trial-inspector margins, canary-suite failures, robustness stress, permutation-null, drift, chronological holdout, and selective-risk evidence into a `blocked`, `needs_review`, or `promotable` verdict. The `Promotion gate lab` preset includes imbalance, shortcut conflicts, tails, and boundary rows so the gate naturally produces useful cautions unless the evidence bundle is strong.

Neighborhood hardness diagnostics run without an active model. They use leave-one-out nearest-neighbor votes to estimate local learnability, rank hard rows, flag ambiguous neighborhoods, and surface rows surrounded mostly by the opposite label. Use it before training to decide whether a dataset needs review, more labels, or a different feature representation. The `Neighborhood hardness lab` preset includes easy class cores, a mixed boundary band, and mislabeled island-style rows so the local vote failures are visible.
...
- Model response diagnostics use partial-dependence-style synthetic rows, so correlated features can create unrealistic combinations. Treat response curves as model-behavior probes, not causal effects.
- Pairwise interaction diagnostics share the same partial-dependence caveat and scan only a capped set of high-variance features by default to keep desktop runs bounded.
...
- Subgroup disparity diagnostics use anonymous numeric feature slices as proxy groups. They can reveal operational error gaps, but they are not a legal fairness audit and cannot identify protected attributes unless your dataset encodes them.
...
- Threshold stability resamples the loaded labeled rows and keeps the active model fixed. It estimates operating-point sensitivity, not a formal external confidence interval.
- Capacity planner uses loaded labels and user-assumed utility values. Treat its budget choice as a local planning aid, not proof that the same precision, lift, or utility will hold on future operational queues.
- Rank lift uses the active model and loaded labels, so top-decile lift and gains tables are validation evidence only. They can be unstable with tiny positive counts, tied scores, or a validation sample that does not match the future review queue.
- Reliability atlas uses empirical probability bins on the loaded rows. Sparse bins are flagged because they make over/underconfidence estimates noisy.
- Calibration slice diagnostics use feature-quantile slices on the loaded labeled rows. They can overstate local calibration risk when slices are small, correlated, or chosen after many scans, so treat them as review targets rather than formal guarantees.
...
- Post-hoc label sensitivity keeps predictions fixed and tests one label flip at a time. It can prioritize human review, but it cannot prove that a label is wrong and should not be used for automatic relabeling.
- Label-noise stress retrains temporary fallback models on synthetically corrupted training labels and scores the original validation split. It is a robustness probe for local validation evidence, not a certified noisy-label training method or a proof that real production label noise follows the simulated rates.
...
- Data acquisition planner is a heuristic collection plan. It cannot know acquisition cost, labeling feasibility, protected attributes, or whether tail rows are valid without domain review.
- Data value scout is a lightweight nearest-neighbor heuristic. It is not exact Data Shapley, influence functions, or causal data valuation, and it cannot decide whether a rare row is valid without domain review.
- Validation plan is a heuristic split recommendation. It does not prove that a future holdout will match production, and row-order risk is inferred from simple first-half/current-half shifts, not from a real timestamp column.
- Validation stability audit estimates sensitivity to repeated stratified partitions. Its folds are correlated, its q10-q90 bands are descriptive empirical ranges rather than confidence intervals, and it cannot replace a genuinely independent or chronological test set.
- Learning curves retrain temporary models and use one fixed internal holdout. They are useful for trend evidence, but still depend on the holdout sample and can be noisy on small datasets.
...
- Leakage sentinel flags statistical leakage/proxy risk, not intent or causality. A high-risk feature may be legitimate if it is truly available at prediction time; a low-risk scan does not prove the feature pipeline is leakage-free.
...
- Neighborhood hardness uses distance in standardized raw-feature space. It is useful for local label consistency, but high-dimensional noisy features or poor scaling can make neighborhoods uninformative.
...
- MPS site-order sweep retrains temporary MPS models and reports validation sensitivity to feature-site order. It does not reorder the active model, does not combine order search with bond-dimension search, and gains below the material-gain threshold should be treated as noise until rerun.
...
- Prior-shift diagnostics assume sensitivity and specificity remain stable while prevalence changes. That is a label-prior shift thought experiment, not proof that covariates, concepts, or calibration remain stable in deployment.
- External holdout evaluation is only as trustworthy as the labeled CSV you choose. It does not tune the active threshold on the holdout; that is intentional so the holdout remains external evidence.
...
- Shadow replay assumes loaded row order carries operational meaning. It does not retrain the model and cannot prove concept drift by itself; use it as a fixed-model monitoring screen before deeper chronological or drift validation.
...
- MPS and tensor-train models treat inputs as ordered sites/cores, so tabular column order becomes part of the hypothesis class. The MPS backend and site-order sweep are informed by tensor-network supervised learning ([Supervised Learning with Tensor Networks](https://arxiv.org/abs/1605.05775)) and tensor-train neural compression ([Tensorizing Neural Networks](https://arxiv.org/abs/1509.06569)); Italtensor keeps the desktop diagnostic bounded by retraining only a few deterministic candidate orders.
...
- Reliability atlas follows reliability-diagram practice: compare observed bin accuracy with mean predicted confidence, report expected/max calibration error, and separate sparse-bin uncertainty from systematic overconfidence or underconfidence.
- Calibration slice diagnostics combine reliability-diagram thinking with slice-based model reporting: compare local mean probability with local prevalence across raw feature ranges, rank by weighted calibration impact, and surface the worst pockets without automatically changing the model ([scikit-learn calibration](https://scikit-learn.org/stable/modules/calibration.html), [On Calibration of Modern Neural Networks](https://arxiv.org/abs/1706.04599), [Model Cards for Model Reporting](https://arxiv.org/abs/1810.03993)).
- Label-noise stress is informed by noisy-label and label-quality research: confident learning focuses on estimating and ranking label issues ([JAIR](https://jair.org/index.php/jair/article/view/12125)), broader noisy-label surveys organize robust-loss, sample-selection, and correction strategies ([arXiv survey](https://arxiv.org/abs/2007.08199)), and methods such as generalized cross entropy and co-teaching show why controlled label corruption can expose brittle training dynamics ([GCE](https://arxiv.org/abs/1805.07836), [Co-teaching](https://proceedings.neurips.cc/paper/8072-co-teaching-robust-training-o)). Italtensor keeps the desktop version bounded by stress-testing temporary NumPy models instead of adding a new noisy-label training dependency.
...
- Prior-shift diagnostics use Bayes-rule predictive-value relationships between prevalence, sensitivity, and specificity, and are informed by class-prior/label-shift work such as Saerens, Latinne, and Decaestecker's classifier-output adjustment procedure and black-box label-shift estimation from Lipton, Wang, and Smola ([Saerens et al. 2002](https://www.isys.ucl.ac.be/staff/marco/Publications/Saerens2002a.pdf), [BBSE](https://arxiv.org/abs/1802.03916), [PMLR paper](https://proceedings.mlr.press/v80/lipton18a.html), [Unified View of Label Shift Estimation](https://arxiv.org/abs/2003.07554)).
- External holdout evaluation follows standard model-evaluation practice: keep a labeled dataset outside fitting/tuning, score it with the already chosen operating threshold, and report both threshold metrics and proper probability scores such as Brier/log loss ([scikit-learn model evaluation](https://scikit-learn.org/stable/modules/model_evaluation.html), [scikit-learn cross-validation and held-out evaluation](https://scikit-learn.org/stable/modules/cross_validation.html), [Model Cards for Model Reporting](https://arxiv.org/abs/1810.03993)).
...
- Shadow replay follows the prequential/progressive-validation habit from streaming ML: score examples in sequence and inspect performance over ordered windows. This mirrors the progressive-validation framing used by streaming libraries such as [River](https://riverml.xyz/0.18.0/api/evaluate/progressive-val-score/) while Italtensor keeps the first version fixed-model and dependency-free, closer to a local monitoring replay than an online learner.
...
- Data acquisition planner follows active-learning and data-centric model-improvement practice: target scarce classes, boundary-like rows, and coverage gaps before spending more compute on tuning. Italtensor keeps this as a transparent heuristic plan rather than an automatic labeler, informed by uncertainty sampling and coverage-oriented data acquisition ideas such as Burr Settles' [Active Learning Literature Survey](https://burrsettles.com/pub/settles.activelearning.pdf).
- Data value scout is informed by data-valuation and influence-analysis research, especially Data Shapley for valuing training examples, efficient nearest-neighbor Shapley-style valuation, and influence functions for tracing predictions back to training rows. Italtensor uses a bounded nearest-neighbor proxy instead of full retraining or Hessian-based influence estimation so the desktop workflow stays fast and dependency-light ([Data Shapley](https://proceedings.mlr.press/v97/ghorbani19c.html), [Efficient Task-specific Data Valuation for Nearest Neighbor Algorithms](https://www.vldb.org/pvldb/vol12/p1610-jia.pdf), [Influence Functions](https://proceedings.mlr.press/v70/koh17a.html)).
- Validation plan follows scikit-learn's cross-validation guidance: preserve stratification for classification, prefer cross-validation when data is scarce, and avoid shuffled-only estimates when sample order carries temporal or deployment meaning ([cross-validation guide](https://scikit-learn.org/stable/modules/cross_validation.html), [`StratifiedKFold`](https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.StratifiedKFold.html), [`TimeSeriesSplit`](https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.TimeSeriesSplit.html)).
- Validation stability uses repeated stratified folds while keeping threshold calibration inside each outer training partition ([`RepeatedStratifiedKFold`](https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.RepeatedStratifiedKFold.html)). Its ranges are deliberately described as empirical sensitivity because no generally unbiased estimator exists for the variance of k-fold cross-validation ([Bengio and Grandvalet, 2004](https://www.jmlr.org/papers/volume5/grandvalet04a/grandvalet04a.pdf)).
- Learning curve diagnostics follow the standard model-selection habit of plotting validation performance against training-set size to distinguish data-limited, underfit/noisy, and stable regimes while keeping validation data separate from the fitted subset ([scikit-learn learning curves](https://scikit-learn.org/stable/modules/learning_curve.html)).
- Leakage sentinel follows target-leakage practice: features that encode target information unavailable at prediction time can inflate validation estimates and fail in production. Italtensor keeps this as a model-free column screen using one-feature separability and low-cardinality label mappings, informed by scikit-learn's [data leakage common pitfalls](https://scikit-learn.org/stable/common_pitfalls.html#data-leakage) and leakage detection/avoidance work such as [Leakage in Data Mining](https://www.cs.umb.edu/~ding/history/470_670_fall_2011/papers/cs670_Tran_PreferredPaper_LeakingInDataMining.pdf).
...
- Neighborhood hardness follows the instance-hardness idea used in dataset difficulty analysis: examples that local classifiers consistently get wrong or split votes on are often ambiguous, mislabeled, or underrepresented. Italtensor keeps the desktop version transparent with leave-one-out kNN votes instead of training a separate hardness model.
...
- Post-hoc label sensitivity is informed by label-noise and confident-learning research: noisy labels can distort training and evaluation, while model behavior can help prioritize annotation review. Italtensor uses a bounded fixed-prediction counterfactual-label diagnostic rather than automatic correction, aligning with the review-first caution in [Confident Learning](https://jair.org/index.php/jair/article/view/12125) and noisy-label survey work ([arXiv survey](https://arxiv.org/abs/2007.08199)).
...
- Threshold stability combines that decision-threshold framing with bootstrap resampling, following the basic nonparametric bootstrap idea from Efron and Tibshirani's [An Introduction to the Bootstrap](https://doi.org/10.1201/9780429246593), to make cutoff sensitivity visible without retraining the model.
- Capacity planner follows ranked-retrieval evaluation practice: top-k precision/recall and lift make finite-review queues easier to reason about than a single threshold alone, while scikit-learn's [precision-recall guidance](https://scikit-learn.org/stable/auto_examples/model_selection/plot_precision_recall.html) keeps the emphasis on positive-class retrieval when positives are rare.
- Rank lift follows cumulative gains and lift-chart practice for binary classifiers: sort by score, compare captured positives against random targeting, and inspect decile response/lift before assigning scarce review capacity. The implementation is informed by current cumulative-gain/lift documentation and resource-constrained classifier evaluation work ([scikit-plots cumulative gain](https://scikit-plots.github.io/stable/modules/generated/scikitplot.kds.plot_cumulative_gain.html), [IBM cumulative gains and lift charts](https://www.ibm.com/docs/en/spss-statistics/32.0.0?topic=overtraining-cumulative-gains-lift-charts), [DataRobot cumulative charts](https://docs.datarobot.com/en/docs/classic-ui/modeling/analyze-models/evaluate/roc-curve-tab/cumulative-charts-classic.html), [Lift Up and Act!](https://arxiv.org/abs/1906.03374)).
