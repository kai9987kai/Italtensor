...
Promotion gate is the pre-save readiness check. It combines dataset size, class balance, validation metrics, calibration, threshold gain, dataset triage, experiment-advisor recommendations, trial-inspector margins, robustness stress, permutation-null, drift, chronological holdout, and selective-risk evidence into a `blocked`, `needs_review`, or `promotable` verdict. The `Promotion gate lab` preset includes imbalance, shortcut conflicts, tails, and boundary rows so the gate naturally produces useful cautions unless the evidence bundle is strong.
