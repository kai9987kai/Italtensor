...
Dataset triage is the fastest pre-training check: it runs the dataset audit, feature separability lens, prototype audit, neighborhood hardness, and model-free OOD sentinel together, then compresses the findings into a readiness score, risk level, blocker count, and prioritized next actions. Running triage also stores the component reports so a later report or model sidecar can include the detailed evidence. The `Dataset triage lab` preset intentionally includes conflicts, a constant column, redundant features, tails, and hard neighborhoods so the action queue is visible.
...
Experiment advisor is the next-step planner. It reads the current dataset, triage report, validation metrics, trial history, and any diagnostics you have run, including calibration-slice and external-holdout evidence, then ranks concrete follow-up experiments: clean data first, train a baseline, expand the search, tune thresholds, repair calibration, test robustness, or validate against drift. Fresh calibration-slice or external-holdout results invalidate older advisor and promotion-gate summaries so exports do not mix new evidence with stale recommendations. The `Experiment advisor lab` preset has nonlinear weak single-feature structure, imbalance, boundary flips, and outliers so the advisor naturally recommends triage plus RFF/SMOTE/CV-style runs.
...
Promotion gate is the pre-save readiness check. It combines dataset size, class balance, validation metrics, calibration, threshold gain, dataset triage, experiment-advisor recommendations, trial-inspector margins, canary-suite failures, robustness stress, permutation-null, drift, chronological holdout, and selective-risk evidence into a `blocked`, `needs_review`, or `promotable` verdict. The `Promotion gate lab` preset includes imbalance, shortcut conflicts, tails, and boundary rows so the gate naturally produces useful cautions unless the evidence bundle is strong.
