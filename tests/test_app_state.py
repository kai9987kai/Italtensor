...
def test_compatible_data_acquisition_report_rejects_mismatched_or_reordered_dataset():
    state = AppState(features=[[0.1], [0.9], [0.2], [0.8]], labels=[0, 1, 0, 1], input_dim=1)
    matching = {
        "dataset_fingerprint": data_acquisition_dataset_fingerprint(state.features, state.labels),
        "summary": {"priority": "low"},
    }
    reordered = {
        "dataset_fingerprint": data_acquisition_dataset_fingerprint([[0.2], [0.1], [0.8], [0.9]], [0, 0, 1, 1]),
        "summary": {"priority": "high"},
    }
...
def test_compatible_label_sensitivity_report_rejects_mismatched_or_reordered_dataset():
    state = AppState(features=[[0.1], [0.9], [0.2], [0.8]], labels=[0, 1, 0, 1], input_dim=1)
    matching = {
        "dataset_fingerprint": label_sensitivity_dataset_fingerprint(state.features, state.labels),
        "summary": {"priority": "low"},
    }
    reordered = {
        "dataset_fingerprint": label_sensitivity_dataset_fingerprint([[0.2], [0.1], [0.8], [0.9]], [0, 0, 1, 1]),
        "summary": {"priority": "high"},
    }
...
def test_handle_worker_done_stores_data_acquisition_without_mutating_model():
    ...
    report = {
        "summary": {
            ...
            "recommended_next_step": "Collect more minority examples.",
        },
        "recommendations": [
            {
                "rank": 1,
                ...
                "title": "Collect more class 1 labels",
                "action": "Collect more minority examples.",
            }
        ],
        "row_candidates": [{"row_index": 3, "candidate_type": "boundary", "score": 0.88}],
    }

    _handle_worker_done(window, state, ("data_acquisition", report))

    assert state.model is model
    assert state.latest_metrics == {"f1": 0.9}
    assert state.latest_data_acquisition_report == report
    assert state.latest_experiment_advisor_report is None
    assert state.latest_promotion_gate_report is None
    assert state.busy is False
...
def test_start_promotion_gate_uses_latest_policy_guard_report():
    ...
    state = AppState(
        ...
        latest_label_sensitivity_report={
            "summary": {
                "priority": "high",
                "suspect_label_count": 2,
                "max_improving_f1_delta": 0.08,
...
def test_handle_worker_done_stores_label_sensitivity_without_mutating_model():
    window = FakeWindow()
    state = AppState(
        model=object(),
        latest_metrics={"f1": 0.9},
    ...
    report = {
        ...
        "anchor_rows": [{"row_index": 1, "label": 0, "probability": 0.05, "f1_delta_if_flipped": -0.08}],
    }

    _handle_worker_done(window, state, ("label_sensitivity", report))

    assert state.model is model
    assert state.latest_metrics == {"f1": 0.9}
    assert state.latest_label_sensitivity_report == report
    assert state.latest_promotion_gate_report is None
    assert state.latest_threshold == 0.4
    assert state.busy is False
        ...
        "recommendations": [
            {
                ...
                "title": "Run repair",
                "action": "Run Calibration repair.",
            }
        ],
    }

    _handle_worker_done(window, state, ("reliability_atlas", report))
