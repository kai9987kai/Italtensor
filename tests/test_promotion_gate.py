...
def test_promotion_gate_cautions_on_high_priority_data_value_scout():
    report = build_promotion_gate(
        sample_count=120,
        input_dim=3,
        labels=[0, 1] * 60,
    ...
    assert report["summary"]["verdict"] == "needs_review"
    assert any(item["title"] == "Data value scout has high-priority row curation" for item in report["checks"])
    assert "data-curation note" in report["release_note"]["must_include"]


def test_promotion_gate_cautions_on_high_priority_label_sensitivity():
    report = build_promotion_gate(
        sample_count=120,
        input_dim=3,
        labels=[0, 1] * 60,
    ...
    assert report["summary"]["verdict"] == "needs_review"
    assert any(item["title"] == "Label sensitivity has high-priority review rows" for item in report["checks"])
    assert "label-quality note" in report["release_note"]["must_include"]


def test_promotion_gate_blocks_severe_label_noise_stress_collapse():
    report = build_promotion_gate(
        sample_count=120,
        input_dim=3,
        labels=[0, 1] * 60,
        ...
        label_noise_stress_report={
            "summary": {
                "priority": "high",
                "worst_mean_f1_drop": 0.18,
                "first_material_noise_rate": 0.10,
                "recommended_next_step": "Audit labels before promotion.",
            }
        },
    )

    assert report["summary"]["verdict"] == "blocked"
    assert any(item["title"] == "Label-noise stress shows severe metric collapse" for item in report["checks"])
    assert "label-quality note" in report["release_note"]["must_include"]
